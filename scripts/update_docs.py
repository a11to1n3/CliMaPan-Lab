...
def update_api_docs(project_root):
    ...
    result = run_command(
        f"sphinx-apidoc -f -o {api_dir} {source_dir}",
        cwd=project_root,
        check=False
    )
...
def main():
    ...
    parser.add_argument("--update-api", action="store_true", help="Update API documentation")
    parser.add_argument("--trigger-rtd", action="store_true", help="Trigger Read the Docs build")
    parser.add_argument("--check-coverage", action="store_true", help="Check docstring coverage")
    ...
    if success:
        ...
        print("🌐 Online docs should be available at: https://climapan-lab.readthedocs.io/")
    else:
