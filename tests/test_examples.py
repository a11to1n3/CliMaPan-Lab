...
class TestExamples(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.examples_dir = os.path.join(os.path.dirname(__file__), '..', 'climapan_lab', 'examples')
    ...
    def test_simple_example_imports(self):
        ...
        # Load the module
        spec = importlib.util.spec_from_file_location("simple_example", simple_example_path)
    ...
    def test_no_personal_paths(self):
        ...
        personal_indicators = [
            'duypham', 'Users', 'Documents', 'Personal Documents',
            'Uni Bochum', 'Work', '/home/', 'C:\\Users\\'
        ]
        
        example_files = [
            'simple_example.py', 'scenario.py', 'Load_data.py',
            'graph.py', 'scenario_testing_new.py'
        ]
    ...
    def test_generic_paths_only(self):
        ...
        acceptable_paths = [
            'results', '../results', './results', 'figures', '../figures', './figures',
            'data', '../data', './data', 'output', '../output', './output'
        ]
        ...
        for example_file in example_files:
            ...
            if os.path.exists(file_path):
                ...
                lines_with_paths = [line for line in content.split('\n') 
                ...
                for line in lines_with_paths:
                    ...
                    has_acceptable_path = any(path in line for path in acceptable_paths + ['figures'])
                    if not has_acceptable_path and ('folder' in line or 'path' in line):
                        print(f"Warning: Potentially non-generic path in {example_file}: {line.strip()}")
...
class TestAnalysisScripts(unittest.TestCase):
    ...
    def setUp(self):
        """Set up test fixtures."""
        self.analysis_dir = os.path.join(os.path.dirname(__file__), '..', 'climapan_lab', 'analysis')
