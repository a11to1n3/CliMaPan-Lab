...
def run_simulation(params: dict, n_years: int = 10) -> dict:
    ...
    for step in range(steps):
        ...
        if hasattr(model, 'climateModule') and hasattr(model.climateModule, 'EM'):
            monthly_co2.append(model.climateModule.EM[-1] if len(model.climateModule.EM) > 0 else 0)
...
def objective_function(params: dict, target_data: pd.DataFrame, n_years: int = 10) -> float:
    """
    ...
    try:
        ...
        for metric in ['GDP', 'UnemploymentRate', 'Investment', 'Climate C02']:
            ...
            metric_distance = 0.6 * acf_distance + 0.25 * cv_distance + 0.15 * trend_distance
...
def bayesian_optimization(
    ...
    for i in range(n_calls):
        ...
        print(f"[{i+1}/{n_calls}] ({phase}) Score: {objective:.6f} | Best: {best_obj:.6f} | {trial_time:.1f}s")
...
def save_results(results: list, output_path: str):
    ...
    for r in results[:10]:
        serializable_results.append({
            ...
            'objective': float(r['objective']) if r['objective'] != float('inf') else 1e10,
            'time': float(r['time']),
...
if __name__ == "__main__":
    ...
    with open(best_params_path, 'w') as f:
        f.write("# Optimized parameters from Autocorrelation-Based Bayesian calibration\n")
