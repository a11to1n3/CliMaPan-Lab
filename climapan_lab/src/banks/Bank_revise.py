...
class Bank(ap.Agent):
    ...
    def agentAssign(self):
        ...
        updateCSFList = self.model.csfirm_agents.select(self.model.csfirm_agents.loan_demand > 0)
        updateCPList = self.model.cpfirm_agents.select(self.model.cpfirm_agents.loan_demand > 0)
    ...
    def _calculate_firm_networth(self, agent):
        ...
        if agent.getNetWorth() < 0:
            self.NPL += -np.sum(agent.getNetWorth())
            self.profit -= (1+agent.iL) * np.sum(agent.loanList)
    ...
    def _calculate_running_loan(self, agent_id):
        ...
        L_CAR = 0  
        if bankruptFraction <= self.totalLoanSupply and self.runningLoan <= self.totalLoanSupply:
    ...
    def sommaW(self, eps=1e-8):
        ...
        if self.R_cb < self.p.gammaRRR * self.deposits:
            ...
            self.F_cb += self.R_cb - self.deposits - self.loans + self.equities #request funding from CB
        ...
        self.agentList = self.model.csfirm_agents + self.model.cpfirm_agents + self.model.greenEFirm + self.model.brownEFirm
        self.numberOfUnemployed = sum(np.array(self.model.aliveConsumers.getEmploymentState() == 0) * np.array((self.model.aliveConsumers.consumerType == 'workers' and self.model.aliveConsumers.getCovidStateAttr('state') != 'dead')))
        self.numberOfEmployed = sum(np.array(self.model.aliveConsumers.getEmploymentState() == 1) * np.array((self.model.aliveConsumers.consumerType == 'workers' and self.model.aliveConsumers.getCovidStateAttr('state') != 'dead')))
        self.profit = 0
        self.totalLoanDemands = 0
        self.equities = 0
        self.actualSuppliedLoan = 0
        ...
        [self._calculate_consumer_networth(agent) for agent in self.model.aliveConsumers]
        ...
        #self.totalLoanSupply = (1/(self.p.bankEquityEta + eps)) * self.equity - self.totalBankruptFraction
