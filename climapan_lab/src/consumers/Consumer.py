...
class Consumer(ap.Agent):
    ...
    def setup(self):
        ...
        #parameter
        self.consumptionSubsistenceLevel = self.p.subsistenceLevelOfConsumption #physical good
    ...
    def desired_C(self):
        ...
        base_consumption = self.consumptionSubsistenceLevel + np.min([self.employed * self.worker_additional_consumption + self.owner * self.owner_additional_consumption, 0.8 * self.deposit / self.price])
        
        self.growth_factor *= (1+ self.consumption_growth)
        self.desired_consumption = self.growth_factor * base_consumption - sick_reduction * sick
        #if sick == True:
            #print("sick reduction", sick, sick_reduction)

    def update_wealth(self):   
        self.income = self.deposit * self.iD + self.isWorker() * self.getWage() + self.owner * (self.p.minimumWage + self.getDiv())
        #if self.owner: print("owner income", self.income)
        self.deposit += self.income - self.consumption #consumption is value variable, updated in transaction
    ...
    def getCovidStateAttr(self, attr):
        if attr in list(self.covidState.keys()):
            ...
            raise SyntaxError('No such attribute. Please select from: state, t, duration, nextState')
        ...
        if inf_f > 0 or inf_c > 0:   
            ...
            if np.random.rand() <= p_infection:
                self.setCovidState('exposed', self.model.t,
                                        lognormal(self.p.T_susceptible_mild_mean,
                                                    self.p.T_susceptible_mild_std),
                                        'mild')
        ...
        if self.getCovidStateAttr('duration') is None:
            if self.getAgeGroup() == "young":
                if np.random.rand() < self.p.p_exposed_mild_young:
                    self.setCovidState('exposed', self.getCovidStateAttr('t'), lognormal(self.p.T_exposed_mild_mean, self.p.T_exposed_mild_std), 'mild')

                else:
                    self.setCovidState('infected non-sympotomatic', self.getCovidStateAttr('t'), lognormal(self.p.T_nonsym_recovered_mean, self.p.T_nonsym_recovered_std), 'recovered')
            ...
            if self.model.t >= self.getCovidStateAttr('t') + self.getCovidStateAttr('duration'):
    ...
    def _progressCovidMildState(self):
        ...
        if self.getCovidStateAttr('duration') is None:
            if self.getAgeGroup() == "young":
                if np.random.rand() < self.p.p_mild_severe_young * ((1 - self.p.p_vax)**int(self.p.covid_settings == 'VAX')):
                    self.setCovidState('mild', self.model.t, lognormal(self.p.T_mild_severe_mean, self.p.T_mild_severe_std), 'severe')
                else:
                    self.setCovidState('infected non-sympotomatic', self.model.t, lognormal(self.p.T_mild_recovered_mean, self.p.T_mild_recovered_std), 'recovered')
            elif self.getAgeGroup() == "working":
                if np.random.rand() < self.p.p_mild_severe_working * ((1 - self.p.p_vax)**int(self.p.covid_settings == 'VAX')):
                    self.setCovidState('mild', self.model.t, lognormal(self.p.T_mild_severe_mean, self.p.T_mild_severe_std), 'severe')
                else:
                    self.setCovidState('infected non-sympotomatic', self.model.t, lognormal(self.p.T_mild_recovered_mean, self.p.T_mild_recovered_std), 'recovered')
            elif self.getAgeGroup() == "elderly":
                if np.random.rand() < self.p.p_mild_severe_elderly * ((1 - self.p.p_vax)**int(self.p.covid_settings == 'VAX')):
                    self.setCovidState('mild', self.model.t, lognormal(self.p.T_mild_severe_mean, self.p.T_mild_severe_std), 'severe')
                else:
                    self.setCovidState('infected non-sympotomatic', self.model.t, lognormal(self.p.T_mild_recovered_mean, self.p.T_mild_recovered_std), 'recovered')
        else:
        ...
        if self.getCovidStateAttr('duration') is None:
            if self.getAgeGroup() == "young":
                if np.random.rand() < self.p.p_severe_critical_young:
                    self.setCovidState('severe', self.model.t, lognormal(self.p.T_severe_critical_mean, self.p.T_severe_critical_std), 'critical')
                else:
                    self.setCovidState('severe', self.model.t, lognormal(self.p.T_severe_recovered_mean, self.p.T_severe_recovered_std), 'recovered')
            elif self.getAgeGroup() == "working":
        ...
        if self.getCovidStateAttr('duration') is None:
            if self.getAgeGroup() == "young":
                if np.random.rand() < self.p.p_critical_death_young:
                    self.setCovidState('critical', self.model.t, lognormal(self.p.T_critical_death_mean, self.p.T_critical_death_std), 'dead')
                else:
                    self.setCovidState('critical', self.model.t, lognormal(self.p.T_critical_recovered_mean, self.p.T_critical_recovered_std), 'recovered')
            elif self.getAgeGroup() == "working":
    ...
    def progressCovid(self):
        if self.getCovidStateAttr('state') == 'exposed':
            ...
            if np.random.rand() < self.p.p_mutation * ((1 - self.p.p_vax)**int(self.p.covid_settings == 'VAX')):
                self.setCovidState('exposed', self.model.t)
            else: self._progressCovidImmunizedState()
