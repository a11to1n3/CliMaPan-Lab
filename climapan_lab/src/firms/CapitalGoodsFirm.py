...
class CapitalGoodsFirm(GoodsFirmBase):
    ...
    def calculate_input_demand(self):
        ...
        self.planned_production = (beta* self.sale_record + (1 - beta) * self.get_aggregate_demand()) + self.capital_increase
    ...
    def produce(self):
        ...
        sick_ratio = np.min([1, np.max([0, aggSickLeaves / (720 * len(self.workersList))])])
        ...
        if self.p.verboseFlag:
            print("capital input", self.get_energy(), self.labour_demand, self.get_capital())
        ...
        self.set_actual_production(production_value - self.capital_increase) #capital firm automatically take away the capital they need for own expansion
    ...
    def price_setting(self):
        ...
        self.set_capital_price(self.price) #for capital firm, capital price is their own price (basically net zero)
    ...
    def update_capital_growth(self):
        self.capital_growth = self.get_capital() * (self.capital_growth_rate + self.capital_depreciation)
