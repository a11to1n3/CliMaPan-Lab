...
class GoodsFirmBase(ap.Agent):
    ...
    def setup(self):
        ...
        self.soldProducts = 1
        self.workersList = []
        [self.workersList.append(aConsumer.id) for aConsumer in self.model.aliveConsumers if aConsumer.consumerType == 'workers' and aConsumer.getBelongToFirm() == self.id and aConsumer.getCovidStateAttr('state') != 'dead']
    ...
    def production_function(self, inputs):
        ...
        production = ( \
                                self.beta_capital \
                                * (capital_input)**self.eta_production \
                                + self.beta_labour \
                                * (labour_input * self.rho_labour)**self.eta_production \
                                + self.beta_energy \
                                * (energy_input * self.rho_energy)**self.eta_production \
                                ) \
    ...
    def production_budgeting(self):
        ...
        self.loan_demand = np.max([self.get_average_production_cost() * self.planned_production - self.deposit * (1 - self.reserve_ratio), self.get_average_production_cost() * self.planned_production * 0.2])
    ...
    def calculate_average_production_cost(self):
        ...
        mean_wage = np.mean(list(self.consumersList.select(self.consumersList.consumerType == 'workers').getWage()))
        ...
        self.average_production_cost = (mean_wage * self.getNumberOfLabours() + energy_cost + cost_of_capital) / (self.get_actual_production())
    ...
    def setBankruptcy(self):
        if self.getBankrupt() == True or self.getNetWorth() < 0:
            ...
            if "ConsumerGoods" in str(self):
                ...
                if np.sum(self.model.csfirm_agents.getUseEnergy() == 'brown') > 2 and np.sum(self.model.csfirm_agents.getUseEnergy() == 'green') > 2:
                    if np.random.rand() < 0.5:
                        for wid in self.workersList:
                            self.model.aliveConsumers.select(self.model.aliveConsumers.getIdentity() == wid).receiveFiring()
                ...
                if np.sum(self.model.cpfirm_agents.getUseEnergy() == 'brown') > 1 and np.sum(self.model.cpfirm_agents.getUseEnergy() == 'green') > 1:
                    if np.random.rand() < 0.5:
                        for wid in self.workersList:
                            self.model.aliveConsumers.select(self.model.aliveConsumers.getIdentity() == wid).receiveFiring()
                        self.useEnergyType('brown')
    ...
    def calculate_all_wages(self):
        ...
        for aConsumer in self.consumersList:
            ...
            self.daysInMonth = days_in_month(int(str(self.model.today).split('-')[1]), int(str(self.model.today).split('-')[0]))
            ...
            if aConsumer.getEmploymentState():
                if self.model.t < 32:
                    wage = (wages / days) * (days - len(sick_leaves)) + (unemployment_dole / days) * len(sick_leaves)
                    if self.lockdown:
                        wage = (wages / days) * (days - lockdown_days) + pandemic_transfer * lockdown_days
                ...
                wage = wages / (1 - self.p.incomeTaxRate) * self.wage_factor * (1 - self.p.incomeTaxRate)
                ...
                self.wage_bill += np.sum(aConsumer.getWage() - (unemployment_dole / days) * len(sick_leaves))
        ...
        energy = (1 / self.rho_energy) * \
                        ((self.planned_production ** self.eta_production - \
                          (self.beta_labour * (self.rho_labour * labour) ** self.eta_production) - \
                            (self.beta_capital * (capital) ** self.eta_production)) / self.beta_energy) \
                        ** (1 / self.eta_production)
        if ((self.planned_production ** self.eta_production - \
                          (self.beta_labour * (self.rho_labour * labour) ** self.eta_production) - \
                            (self.beta_capital * (capital) ** self.eta_production)) / self.beta_energy) < 0:
            energy = 0
        return energy
    ...
    def fire(self):
        ...
        for id in self.workersList: #this line require an update mechanic to fire worker since the worker list shrink as we fire
            worker = self.model.aliveConsumers.select(self.model.aliveConsumers.getIdentity() == id)
