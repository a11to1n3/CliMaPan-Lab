...
def percentage_change(base, compare):
    percentage_diff = [(b - a) / a * 100 if a != 0 else 0 for a, b in zip(base, compare)]
...
def clean_data(df, column_name):
    ...
    """
    df[f"{column_name}_cleaned"] = df[column_name].apply(lambda value_list: [x for x in value_list if x is not None])

def calculate_average(df, column_name):
    """
    Calculate the average of the specified column in the DataFrame across all lists at each time step.
    """
    max_length = max(len(value_list) for value_list in df[f"{column_name}_cleaned"])  # Find the maximum length of the lists
    avg_values = [
        np.mean([value_list[i] for value_list in df[f"{column_name}_cleaned"] if len(value_list) > i]) 
...
def calculate_confidence_interval(df, column_name, confidence=0.95):
    ...
    for i in range(max_length):
        values_at_t = [value_list[i] for value_list in df[f"{column_name}_cleaned"] if len(value_list) > i]
...
def sensitivity_3d(name, data, length, covid_time, dataset_name, savecode, length_cut = 10, area =False, export = False):
    ...
    if export:
        ...
        pio.write_image(fig, os.path.join(folder_path, savecode + '_' + str(name) +'_plot.pdf'), format='pdf')

def scatter_3d(name, data, length, dataset_name, savecode, length_cut = 10, area =False, export = False):
    ...
    if length_cut != None:
        ...
        start_date = end_date - pd.DateOffset(months=length)  # Calculate the start date 48 months before
        date_range = pd.date_range(start=start_date, periods=length_cut, freq='MS')  # Generate the date range
        ntick = length_cut
...
def pline_plot(name, data, color_set, length, covid_time, dataset_name, savecode, graph_title, length_cut=None, tick= 3, area=False, export=False, is_percentage=False, apply_hp_filter=False, hp_lambda=14400):
    folder_path = r'Figures'  # Saving folder
    x = []
    ...
    fig = go.Figure()
    for i, item in enumerate(x):
        fig.add_trace(go.Scatter(x=date_range, y=item, name=dataset_name[i], mode='lines', marker=dict(color=color_set[i])))
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            title=dict(text=y_axis_title, font=dict(family="Times New Roman, serif", size=20, color="black")),
            tickwidth=2,
            ticklen=10,
            gridcolor='lightgray',
...
def pline_plot_with_ci(name, data, color_set, length, covid_time, dataset_name, savecode, graph_title, ci_data, length_cut=None, tick= 3, area=False, export=False):
    """
    ...
    if length_cut is not None:
        end_date = pd.to_datetime('2023-01-01')  # The given end date
        start_date = end_date - pd.DateOffset(months=length)  # Calculate the start date months before
        date_range = pd.date_range(start=start_date, periods=length_cut, freq='MS')  # Generate the date range
        ntick = length_cut
    else:
        date_range = pd.date_range(end='2023-01-01', periods=length, freq='MS')
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            title=dict(text=name, font=dict(family="Times New Roman, serif", size=20, color="black")),
            tickwidth=2,
            ticklen=10,
            gridcolor='lightgray',
            zerolinecolor='lightgray',
...
def pbar_plot(name, data, length, covid_time, dataset_name, savecode, area = False, export = False):
    ...
    fig.update_layout(
        xaxis=dict(
            title=dict(text="Comparison Plot", font=dict(family="Times New Roman, serif", size=20, color="black")),
            tickmode="array",
            tickvals=tickvals,
            ticktext=ticktext,
            tickangle=-90,
    ...
    if area:
        ...
        fig.add_vrect(x0=pd.to_datetime(date_range[0]), x1=pd.to_datetime(covid_time), 
                annotation_text="Pre-Pandemic", annotation_position="top left",
                annotation=dict(font_size=20, font_family="Times New Roman"),
                fillcolor="lightgreen", opacity=0.25, line_width=0)
    if export:
        # Export the plot as a PDF file in the 'figure' subfolder
        pio.write_image(fig, os.path.join(folder_path, str(name) + '_' + savecode +'_plot.pdf'), format='pdf')

    fig.show()

def pradar_plot(name, data, length, covid_time, ):
    # Create a date range with the same length as each item in the variable list
    ...
    data_snapshot = [data[i][date_range == closest_date].values[0] for i in range(len(data))]
...
def covline_plot(name, data, color_set, length, dataset_name, savecode, graph_title, tick = 3, export = False):
    ...
    fig = go.Figure()
    for i, item in enumerate(x):
        fig.add_trace(go.Scatter(x=date_range, y=item,name=name + "_" + dataset_name[i], mode='lines', marker= dict(color = color_set[i])))
...
def box_plot(name, data, color_set, length, dataset_name, savecode, graph_title, length_cut = None, export = False):
    ...
    fig = go.Figure()
    for i, item in enumerate(x):
        fig.add_trace(go.Box(y=item, name=dataset_name[i], marker=dict(color=color_set[i])))
...
def heatmap_plot(name, data, color_set, length, dataset_names, savecode, graph_title, length_cut=None, tick=3, export=False):
    ...
    if export:
        pio.write_image(fig, os.path.join(folder_path, savecode + '_heatmap.pdf'), format='pdf')
