...
def setup_basic_scenario(scenario="BAU", covid_policy="BAU", steps=120, agents_scale=1.0):
    """
    ...
    params.update({
        ...
        'green_energy_owners': max(1, int(params['green_energy_owners'] * agents_scale)),
        'brown_energy_owners': max(1, int(params['brown_energy_owners'] * agents_scale)),
        'steps': steps,
        'settings': scenario,
...
def run_basic_simulation(params, output_dir="results"):
    ...
    """
    print(f"Running simulation: {params['settings']} scenario with {params['covid_settings']} COVID policy")
    print(f"Agents: {params['c_agents']} consumers, {params['csf_agents']} consumer firms, {params['cpf_agents']} capital firms")
...
def create_basic_plots(results, save_folder):
    ...
    if 'UnemploymentRate' in data:
        ...
        unemployment_series = [x for x in unemployment_data if x is not None and str(x) != 'nan']
...
def main():
    ...
    parser.add_argument('--scenario', choices=['BAU', 'CT', 'CTR'], default='BAU',
                       help='Economic scenario to run')
    parser.add_argument('--covid', choices=['BAU', 'DIST', 'LOCK', 'VAX'], default='BAU',
                       help='COVID policy to apply')
    parser.add_argument('--steps', type=int, default=120,
                       help='Number of simulation steps (months)')
    parser.add_argument('--scale', type=float, default=0.1,
                       help='Agent scale factor (0.1 for quick testing, 1.0 for full simulation)')
    parser.add_argument('--output', default='results',
                       help='Output directory for results')
    parser.add_argument('--plots', action='store_true',
                       help='Generate basic plots')
    parser.add_argument('--analysis', action='store_true',
                       help='Perform basic analysis')
