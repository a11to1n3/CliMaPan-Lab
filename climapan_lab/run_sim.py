...
def single_run(parameters, idx=0, parent_folder=None, make_stats=False, var_dict=None, args=None):
    multi_params = False
    if len(parameters) == 2:
        multi_params = True
    ...
    if multi_params:
        varying_params = ''.join([str(list(varying_var.keys())[i])+str(list(varying_var.values())[i]) for i in range(len(varying_var))])
    else:
        varying_params = None

    if args and hasattr(args, 'climateDamage') and args.climateDamage and parameters.get('climateShockMode') is not None:
        shockModeList = parameters['climateShockMode']
        if parent_folder:
            save_folder = os.path.abspath(f"./{parent_folder}/results_{parameters['settings']}_{parameters['covid_settings']}_{''.join(shockModeList)}")
            if varying_params is not None:
                save_folder += f"_{varying_params}"
            save_folder += f"_{timestamp}"
        else:
            save_folder = os.path.abspath(f"./results/results_{parameters['settings']}_{parameters['covid_settings']}_{''.join(shockModeList)}")
            if varying_params is not None:
                save_folder += f"_{varying_params}"
            save_folder += f"_{timestamp}"
    else:
        ...
        if parent_folder:
            save_folder = os.path.abspath(f"./{parent_folder}/results_{parameters['settings']}_{parameters['covid_settings']}")
            if varying_params is not None:
                save_folder += f"_{varying_params}"
            save_folder += f"_{timestamp}"
        else:
            save_folder = os.path.abspath(f"./results/results_{parameters['settings']}_{parameters['covid_settings']}")
            if varying_params is not None:
                save_folder += f"_{varying_params}"
            save_folder += f"_{timestamp}"
    ...
    if varListNpy is not None and len(varListNpy) > 0:
        for var in varListNpy:
            if var in results.variables.EconModel.columns:
                saving_var = np.array([i for i in list(results.variables.EconModel[var.strip()].values) if (i is not None)])
                saving_var = np.array([[i] if 'ndarray' in str(type(i)) else i for i in saving_var])
                np.save(f"{save_folder}/{''.join(var.strip().split(' '))}.npy",np.array([list(i) if ('ndarray' in str(type(i)) and i.shape != ()) else i for i in list(results.variables.EconModel[var.strip()].values)]))
                results.variables.EconModel = results.variables.EconModel.drop(columns=[var.strip()])
            ...
            if var in results.variables.EconModel.columns:
                ...
                for i in results.variables.EconModel[var].values:
                    if i is not None and 'ndarray' in str(type(i)):
                        ...
                        varList.append(i)
                try:
                    pd.DataFrame([i for i in results.variables.EconModel[var].values]).to_csv(f"{save_folder}/{''.join(var.strip().split(' '))}.csv")
                except:
                    pass
    results.variables.EconModel.to_csv(f"{save_folder}/single_run.csv.gz", compression='gzip')
    ...
    if varListNpy is not None and len(varListNpy) > 0:
        for var in varListNpy:
            if var in results.variables.EconModel.columns:
                saving_var = np.array([i for i in list(results.variables.EconModel[var.strip()].values) if (i is not None)])
                saving_var = np.array([[i] if 'ndarray' in str(type(i)) else i for i in saving_var])
                np.save(f"{process_save_path}/{''.join(var.strip().split(' '))}.npy", saving_var)
                results.variables.EconModel = results.variables.EconModel.drop(columns=[var.strip()])
    ...
    if varListCsv is not None and len(varListCsv) > 0:
            for var in varListCsv:
                if var in results.variables.EconModel.columns:
                    ...
                    pd.DataFrame([i for i in results.variables.EconModel[var].values]).to_csv(f"{process_save_path}/{''.join(var.strip().split(' '))}.csv")
...
if __name__ == '__main__':
    ...
    parser.add_argument('-n', '--noOfRuns', type=int, default=1,
                    help='an integer for number of runs')
    parser.add_argument('-s', '--settings', type=str, default='BAU',
                    help='situation settings: BAU, CT, CTRa, CTRb, CTRc, CTRd')
    parser.add_argument('-c', '--covidSettings', type=str, default=None,
                    help='situation settings: BAU, DIST, LOCK, VAX')
    parser.add_argument('-d', '--climateDamage', type=str, default="Flood",
                    help='no situation')
    parser.add_argument('-l', '--extractedVarListPathNpy', default=None,
                    help='extractedVarListPathNpy is the list of variables to be extracted to separate numpy files')
    parser.add_argument('-v', '--extractedVarListPathCsv', default=None,
                    help='extractedVarListPathCsv is the list of variables to be extracted to separate csv files')
    parser.add_argument('-p', '--plot', action='store_true',
                    help='(bool) plot or not')
    ...
    if args.extractedVarListPathNpy is not None and os.path.exists(args.extractedVarListPathNpy.strip()) and args.extractedVarListPathNpy.strip().endswith('.txt') :
    ...
    if args.extractedVarListPathCsv is not None and os.path.exists(args.extractedVarListPathCsv.strip()) and args.extractedVarListPathCsv.strip().endswith('.txt') :
    ...
    if args.noOfRuns == 1:
        ...
        if count == 0:
            ...
            print(f"Given parameter list will be split into {len(list_parameters_combinations)} variants")
            ...
            Parallel(n_jobs=-1, prefer="threads")([delayed(single_run)(params, idx, parent_folder=parent_folder, make_stats=True, var_dict=var_dict) for idx, params in enumerate(parameters_combinations)])
        ...
        if count == 0:
            ...
            Parallel(n_jobs=-1, prefer="threads")([delayed(multi_run)(overall_dict, i, save_folder) for i in range(60,60+args.noOfRuns)])
