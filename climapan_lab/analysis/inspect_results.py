...
def inspect_results(results_path):
    ...
    print('Results attributes:', [attr for attr in dir(results) if not attr.startswith('_')])
    
    if hasattr(results, 'variables'):
        print('\nVariables type:', type(results.variables))
        print('Variables attributes:', [attr for attr in dir(results.variables) if not attr.startswith('_')])
